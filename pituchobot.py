...
bot.set_my_commands([
    ...
    telebot.types.BotCommand("/felicidade", "Medidor de nível de felicidade 🙂"),
    telebot.types.BotCommand("/raiva", "Medidor de nível de raiva 🤬"),
    telebot.types.BotCommand("/estado", "Adivinha em qual estado você mora 🌏"),
    telebot.types.BotCommand("/stickers", "Envia uma figurinha"),
    telebot.types.BotCommand("/presidente", "Adivinha quem é o seu presidente 🇧🷷"),
    telebot.types.BotCommand("/sorte", "Veja a sua sorte"),
    telebot.types.BotCommand("/orientacao", "Adivinha o seu gênero ⚧"),
    telebot.types.BotCommand("/casamento", "Adivinha a data do seu casamento 👨‍❤️‍👨"),
    telebot.types.BotCommand("/nascimento", "Adivinha a data do seu nascimento"),
    telebot.types.BotCommand("/morte", "Adivinha a data da sua morte 😵"),
    telebot.types.BotCommand("/filho", "Adivinha o nome, gênero e quando você vai ter um filho 👶"),
    telebot.types.BotCommand("/signo", "Adivinha o seu signo ♈️"),
    telebot.types.BotCommand("/religiao", "Adivinha a sua religião 🤍"),
    telebot.types.BotCommand("/ator", "Adivinha o seu ator/atriz preferida 🥰"),
    telebot.types.BotCommand("/altura", "Adivinha a sua altura 🤏"),
    telebot.types.BotCommand("/idade", "Adivinha a sua idade 🎯"),
    telebot.types.BotCommand("/sistema", "Adivinha o seu socioeconômico-políticos 🎫"),
    telebot.types.BotCommand("/profissao", "Adivinha a sua profissão 💼"),
    telebot.types.BotCommand("/saudades", "Escolha de 0 a 100 quanto de saudades você tem 😰"),
    telebot.types.BotCommand("/ciumes", "Escolha de 0 a 100 quanto de ciúmes você tem 😠"),
    telebot.types.BotCommand("/apaixonado", "Medidor de nível de paixão 😍"),
    telebot.types.BotCommand("/gado", "Medidor de nível de gado 🐮"),
    telebot.types.BotCommand("/tpm", "Medidor de nível de TPM 😤"),
    telebot.types.BotCommand("/gostosura", "Medidor de nível de gostosura 😈"),
    ...
    telebot.types.BotCommand("/sexo", "Medidor de nível de potencial sexual 🔞"),
    telebot.types.BotCommand("/fake", "Descobre se você é fake 🛃"),
    telebot.types.BotCommand("/serbanido", "Descobre se você vai ser banido 🚷"),
    telebot.types.BotCommand("/seradm", "Descobre se você pode ser adm 👮‍♀️"),
    telebot.types.BotCommand("/filme", "Adivinha a categoria de filme preferido 🎬"),
    telebot.types.BotCommand("/clima", "Adivinha o seu clima preferido 🌪"),
    telebot.types.BotCommand("/numerodasorte", "Gera o seu número da sorte 🎰"),
    telebot.types.BotCommand("/desafio", "Propõe um desafio para você 🏆"),
    telebot.types.BotCommand("/musica", "Adivinha o seu estilo musical preferido 🎼"),
    telebot.types.BotCommand("/jogo", "Envia um jogo aleatório 🎮"),
    telebot.types.BotCommand("/cor", "Adivinha a sua cor preferida 🔴"),
    telebot.types.BotCommand("/crush", "Adivinha a primeira letra do seu crush 🥰"),
    ...
    telebot.types.BotCommand("/curiosidade", "Envia aleatoriamente uma curiosidade sobre o mundo 👀"),
...
def on_bot_added_to_group(message):
    ...
    if not group:
        ...
        bot.send_message(GROUP_LOG, f"<b>#{NAME_BOT} #NEW_GRUP</b>\n<b>Name:</b> {title}\n<b>ID:</b> <code>{chat_id}</code>")
    else:
        # Verifique se chat_banned é igual a "true" no banco de dados
        if group.get('chat_banned') == 'true':
            try:
                bot.leave_chat(chat_id)
                bot.send_message(GROUP_LOG, f"O bot saiu do grupo {title} (ID: {chat_id}) devido a chat_banned = true.")
            except Exception as e:
                bot.send_message(GROUP_LOG, f"Erro ao sair do grupo {title} (ID: {chat_id}): {str(e)}")
        ...
        if len(message.text.split()) == 2:
            ...
            if user:
                if user.get("sudo") == "true":
                    ...
                    result = db.users.update_one({"user_id": user_id}, {"$set": {"sudo": "true"}})
        ...
        if len(message.text.split()) == 2:
            ...
            if user:
                if user.get("sudo") == "false":
                    ...
                    result = db.users.update_one({"user_id": user_id}, {"$set": {"sudo": "false"}})
                    if result.modified_count > 0:
                        if message.from_user.username:
                                username = "@" + message.from_user.username
                        else:
                        ...
                        if updated_user:
                            ...
                            bot.send_message(GROUP_LOG, f"<b>#{NAME_BOT} #Rem_sudo</b>\n<b>ID:</b> <code>{user_id}</code>\n<b>Nome:</b> {updated_user.get('name')}\n<b>Username:</b> {username}")
                    else:
            ...
            if len(message.text.split()) == 2:
                ...
                if ban_user:
                    if ban_user.get("banned") == "true":
                        ...
                        result = db.users.update_one({"user_id": ban_user_id}, {"$set": {"banned": "true"}})
                        if result.modified_count > 0:
                            if message.from_user.username:
                                username = "@" + message.from_user.username
                            else:
                                username = "Não tem um nome de usuário"
                            updated_user = db.users.find_one({"user_id": ban_user_id})
                            if updated_user:
                                bot.send_message(message.chat.id, f"<b>Usuário banido</b>\n\n<b>ID:</b> <code>{ban_user_id}</code>\n<b>nome:</b> {updated_user.get('name')}\n<b>Username: {username}")
                                bot.send_message(GROUP_LOG, f"<b>#{NAME_BOT} #user_banned</b>\n<b>ID:</b> <code>{user_id}</code>\n<b>Nome:</b> {updated_user.get('name')}\n<b>Username: {username}")
                            else:
                                bot.send_message(message.chat.id, "Usuário não encontrado no banco de dados.")
                else:
                    bot.send_message(message.chat.id, "Usuário não encontrado no banco de dados.")
            ...
            if len(message.text.split()) == 2:
                ...
                if unban_user:
                    if unban_user.get("banned") == "false":
                        ...
                        result = db.users.update_one({"user_id": unban_user_id}, {"$set": {"banned": "false"}})
                        if result.modified_count > 0:
                            if message.from_user.username:
                                username = "@" + message.from_user.username
                            else:
                                username = "Não tem um nome de usuário"
                            updated_user = db.users.find_one({"user_id": unban_user_id})
                            if updated_user:
                                bot.send_message(message.chat.id, f"<b>Usuário uban</b>\n\n<code>{unban_user_id}</code>\n<b>Nome:</b> {updated_user.get('name')}\n<b>Username:</b> {username}")
                                bot.send_message(GROUP_LOG, f"<b>#{NAME_BOT} #User_unbanned</b>\n<b>ID:</b> <code>{user_id}</code>\n<b>Nome:</b> {updated_user.get('name')}\n<b>Username:</b> {username}")
                            else:
                                bot.send_message(message.chat.id, "Usuário não encontrado no banco de dados.")
                else:
                    bot.send_message(message.chat.id, "Usuário não encontrado no banco de dados.")
...
def ban_group(message):
    # Verifique se o comando é usado em um chat privado
    if message.chat.type != "private":
        bot.reply_to(message, "Este comando só pode ser usado em chat privado.")
        return

    user_id = message.from_user.id

    # Verifique se o usuário é um administrador (sudo)
    if not sudo(user_id):
        bot.reply_to(message, "Você não tem permissão para usar este comando.")
        return

    # Certifique-se de que a mensagem inclui um argumento (chat_id)
    if len(message.text.split()) != 2:
        bot.reply_to(message, "Uso correto: /ban_gp chat_id")
        return

    # Obtenha o chat_id do argumento
    chat_id = int(message.text.split()[1])

    # Procure o grupo no banco de dados
    group = search_group(chat_id)

    # Verifique se o grupo está no banco de dados
    if group:
        # Atualize o status de chat_banned para "true"
        db.groups.update_one({"chat_id": chat_id}, {"$set": {"chat_banned": "true"}})

        # Tente fazer o bot sair do grupo
        try:
            bot.leave_chat(chat_id)
        except Exception as e:
        ...
        bot.reply_to(message, f"Grupo {chat_id} foi banido e o bot saiu do grupo.")
    else:
...
def start(message):
    ...
    add_to_group_button = types.InlineKeyboardButton("✨ Adicione-me ao seu grupo", url=f"https://t.me/{USERNAME_BOT}?startgroup=true")
    commands_button = types.InlineKeyboardButton("🗃 Lista de Comandos", callback_data="commands")
    info_button = types.InlineKeyboardButton("❓ Informações", callback_data="info")
...
def callback_handler(callback_query):
    ...
    if callback_query.data == "commands":
        ...
        back_button = types.InlineKeyboardButton("⬅️ Voltar", callback_data="back_to_start")
        ...
        markup = types.InlineKeyboardMarkup()
        projects_button = types.InlineKeyboardButton("🗂 Projetos", url="https://t.me/infolbrabo")
        support_button = types.InlineKeyboardButton("👨‍💻 Suporte", url="https://t.me/kylorensbot")
        back_button = types.InlineKeyboardButton("⬅️ Voltar", callback_data="back_to_start")
...
def felicidade(message):
    ...
    emojis = ["😊", "😄", "🤩", "🎉", "🌟", "💖"]  # Adicione mais emojis conforme desejado
    emoji = random.choice(emojis)

    grafico = "🟩" * (felicidade // 10) + "🟩" + "⬜️" * ((100 - felicidade) // 10)
    ...
    try:
        ...
        logging.info(f"Usuário {message.from_user.username} solicitou nível de felicidade: {felicidade}%")
    except Exception as e:
        bot.send_message(message.chat.id, resposta, parse_mode="HTML")
        logging.info(f"Usuário {message.from_user.username} solicitou nível de felicidade, mas a mensagem original foi apagada: {felicidade}%")
...
def raiva(message):
    ...
    grafico = "🟩" * (nivel_raiva // 10) + "🟨" + "🟨" * ((100 - nivel_raiva) // 10)
...
def estado(message):
    ...
    resposta = f"Adivinho que você mora em {estados_br[estado_adivinhado]}! 😄🌏🏠"
    try:
        bot.reply_to(message, resposta, parse_mode="HTML")
    except Exception as e:
        bot.send_message(message.chat.id, resposta, parse_mode="HTML")
...
def presidente(message):
    ...
    resposta = (
        f"🇧🇷 Seu presidente é: {nome}\n"
        f"Partido: {partido}\n"
    )

    try:
      bot.send_photo(message.chat.id, imagem, caption=resposta, reply_to_message_id=message.message_id)
    except Exception as e:
...
def sorte(message):
    probabilidade_certo = random.random()  # Gera um número aleatório entre 0 e 1
    if probabilidade_certo <= 0.15:
        # 15% de chance de cair no certo
        sticker = random.choice(stickers['certo'])
    else:
        # Restante cai no errado
...
# Lista de meses para adivinhar
meses = [
    "Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho",
    "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"
]

# Lista de horas para adivinhar
horas = [
    "8:00", "12:30", "15:45", "18:15", "20:00", "22:30",
    "10:00", "14:30", "17:00", "19:30", "21:00", "23:30"
]
...
# Lista de nomes de bebês
nomes_bebes = ["Ana", "João", "Maria", "Pedro", "Laura", "Lucas", "Julia", "Gabriel", "Clara", "Enzo"]

# Lista de gêneros
generos = ["menino", "menina"]

# Lista de anos para adivinhar
...
def altura(message):
    # Intervalo de alturas (por exemplo, de 140 a 200 cm)
    altura_minima = 140
    altura_maxima = 200
    
    # Gere uma altura aleatória dentro do intervalo
    altura_aleatoria = random.randint(altura_minima, altura_maxima) / 100.0  # Altura em metros
    altura_formatada = f"{altura_aleatoria:.2f}".replace(".", ",")  # Formato com vírgula

    # Escolha um emoji de acordo com a altura
    if altura_aleatoria < 1.60:
        emoji = "🤏"  # Baixa estatura
    elif altura_aleatoria < 1.70:
        emoji = "🚶‍♂️"  # Estatura média
    else:
        emoji = "🏋️‍♂️"  # Alta estatura
...
def idade(message):
    # Intervalo de idades (por exemplo, de 18 a 100 anos)
    idade_minima = 18
    idade_maxima = 100

    # Gere uma idade aleatória dentro do intervalo
    idade_aleatoria = random.randint(idade_minima, idade_maxima)

    # Escolha um emoji de acordo com a idade
...
sistemas = [
  {
    ...
    'descricao':
      "Sistema econômico em que os meios de produção são de propriedade privada e as atividades econômicas são guiadas pelo mercado.",
    'emoji': "💰",
  },
  {
    'nome': "Socialismo",
    'codenome': "Socialista",
    'descricao':
      "Sistema econômico em que os meios de produção são de propriedade coletiva e as atividades econômicas são planejadas e controladas pelo Estado.",
    'emoji': "👥",
  },
  {
    'nome': "Comunismo",
    'codenome': "Comunista",
    'descricao':
      "Sistema político e econômico que busca a eliminação das classes sociais e a propriedade coletiva dos meios de produção.",
    'emoji': "🚩",
  },
  {
    'nome': "Anarquismo",
    'codenome': "Anarquista",
    'descricao':
      "Filosofia política que defende a eliminação do Estado e a organização da sociedade em comunidades autônomas e auto-geridas.",
    'emoji': "Ⓐ",
  },
  {
    'nome': "Fascismo",
    'codenome': "Facista",
    'descricao':
      "Ideologia política autoritária que enfatiza o nacionalismo, o militarismo e o totalitarismo.",
    'emoji': "🕊️",
  },
  {
    'nome': "Monarquia",
    ...
    'descricao':
      "Sistema político em que o poder é exercido por um monarca, geralmente de forma hereditária.",
    'emoji': "👑",
  },
  {
    'nome': "República",
    ...
    'descricao':
      "Sistema político em que o poder é exercido pelo povo ou seus representantes eleitos.",
    'emoji': "🗳️",
  },
  {
    'nome': "Democracia",
    'codenome': "Democrata",
    'descricao':
      "Sistema político em que o poder é exercido pelo povo ou seus representantes eleitos, através de processos democráticos.",
    'emoji': "🗳️",
  },
  {
    'nome': "Teocracia",
    'codenome': "Teocrata",
    'descricao':
      "Sistema político em que o poder é exercido por líderes religiosos ou por uma religião oficial.",
    'emoji': "🕍",
  },
  {
    'nome': "Meritocracia",
    ...
    'descricao':
      "Sistema político em que o poder é exercido por indivíduos com base em seus méritos e habilidades.",
    'emoji': "🎓",
  },
  {
    'nome': "Plutocracia",
    'codenome': "Plutocrata",
    'descricao':
      "Sistema político em que o poder é exercido pelos mais ricos ou pelos proprietários de grandes empresas.",
    'emoji': "💰",
  },
  {
    'nome': "Oligarquia",
    ...
    'descricao':
      "Sistema político em que o poder é exercido por um pequeno grupo de pessoas.",
    'emoji': "👥",
  },
  {
    'nome': "Totalitarismo",
    ...
    'descricao':
      "Sistema político em que o Estado tem controle total sobre a sociedade e a economia.",
    'emoji': "🛡️",
  },
  {
    'nome': "Autocracia",
    'codenome': "Autocrata",
    'descricao':
      "Sistema político em que o poder é exercido por uma pessoa ou por um pequeno grupo de pessoas, sem qualquer participação popular ou limitações constitucionais.",
    'emoji': "👤",
  },
]
...
def comando_fake(message):
    ...
    foto_perfil_usuario = f"https://tg.org/user_{user_id}/profile_photo"  # Exemplo fictício

    # Calcula a probabilidade de ser fake
    probabilidade_fake = calcular_probabilidade_fake(nome_usuario, foto_perfil_usuario)
...
def calcular_probabilidade_banimento(usuario):
    ...
    media_probabilidades = (probabilidade_comportamento + probabilidade_denúncias + probabilidade_atividade_suspeita) / 3
...
def comando_ser_banido(message):
    ...
    probabilidade_banimento = calcular_probabilidade_banimento(message.from_user)
...
def calcular_probabilidade_ser_adm(usuario):
    ...
    media_probabilidades = (probabilidade_contribuicao + probabilidade_responsabilidade + probabilidade_conhecimento) / 3
...
def adivinhar_cor_preferida(message):
    ...
    # Envie a cor preferida adivinhada
...
def adivinhar_primeira_letra_crush(message):
    ...
    # Escolha aleatoriamente uma letra
    primeira_letra = random.choice(letras)

    # Envie a primeira letra adivinhada
...
def adivinhar_time_coracao(message):
    ...
    # Escolha aleatoriamente um time
    time = random.choice(times)

    # Crie uma mensagem com informações do time
    ...
    try:
      bot.send_photo(chat_id, time['imagemUrl'], caption=mensagem, reply_to_message_id=message.message_id)

    except Exception as e:
        # Se ocorrer um erro, envie uma mensagem de erro
...
def adivinhar_fruta_preferida(message):
    ...
    try:
        # Escolha aleatoriamente uma fruta
        fruta = random.choice(frutas)

        # Crie uma mensagem com a fruta escolhida
        ...
        # Se ocorrer um erro, envie uma mensagem de erro
        bot.reply_to(message, f"Ocorreu um erro ao adivinhar a sua fruta preferida: {str(e)}")
...
def enviar_curiosidade(message):
    ...
    try:
        # Escolha aleatoriamente uma curiosidade
        curiosidade = random.choice(curiosidades)

        # Envie a curiosidade como mensagem
        bot.send_message(chat_id, curiosidade)

    except Exception as e:
        # Se ocorrer um erro, envie uma mensagem de erro
        bot.send_message(chat_id, f"Ocorreu um erro ao buscar uma curiosidade: {str(e)}")

# sigma

@bot.message_handler(commands=['sigma'])
def sigma(message):
    # Gera um nível sigma aleatório entre 1 e 100
    nivelSigma = random.randint(1, 100)

    # URLs dos GIFs com base no nível sigma
    gifSigma = None
    if nivelSigma < 30:
    ...
    # Mensagem a ser enviada com o GIF e legenda
    mensagem = f"Seu nível Sigma é {nivelSigma} 🗿🍷"
    mensagem_com_gif = f"{mensagem}\n{gifSigma}"

    # Envia a mensagem com o GIF e legenda
    try:
        
       bot.send_message(message.chat.id, mensagem_com_gif, reply_to_message_id=message.message_id)
    except Exception as e:
